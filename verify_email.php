...
if ($uid > 0 && $token !== '') {
    $result = toolshare_verify_email($pdo, $uid, $token);
...
<head>
    ...
    <title>Verify Email | ToolShare</title>
    <style>
...
<body>
    <div class="card">
        ...
        <p>Use this page to confirm your ToolShare email address and activate your sign-in.</p>
        <div class="msg <?= $status === 'verified' || $status === 'already_verified' ? 'success' : 'error' ?>">
            <?= htmlspecialchars($message) ?>
        </div>
        <div class="actions">
            <a class="btn btn-primary" href="login.php">Go to Login</a>
            <?php if ($status === 'expired' || $status === 'invalid'): ?>
                <a class="btn btn-secondary" href="resend_verification.php">Resend Verification</a>
            <?php endif; ?>
        </div>
    </div>
</body>
