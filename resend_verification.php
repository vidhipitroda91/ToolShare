...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($email === '') {
        ...
        $stmt = $pdo->prepare("SELECT id, full_name, email, role, email_is_verified FROM users WHERE email = ? LIMIT 1");
        $stmt->execute([$email]);
        $user = $stmt->fetch();

        if (!$user || toolshare_normalize_role($user['role'] ?? 'user') !== 'user') {
            $success = 'If an unverified account exists for that email, a new verification email has been sent.';
        } elseif ((int)$user['email_is_verified'] === 1) {
            $success = 'This email is already verified. You can sign in now.';
        } else {
            try {
                toolshare_resend_email_verification($pdo, $user);
...
<head>
    ...
    <title>Resend Verification | ToolShare</title>
    <style>
...
<body>
    <div class="card">
        ...
        <p>Enter the email address you used during signup and we will send a fresh verification link.</p>
        <?php if ($error !== ''): ?>
